...
export default function DetailPageManager({ readOnly=false, managedProducts=[] }) {
  ...
  return <Box sx={{p:{xs:2,md:3}, bgcolor:'#F8F8FA'}}>
    ...
    <Box sx={{display:'grid',gap:2.5}}>
      ...
      <Paper elevation={0} sx={groupSx}><h3>이미지·영상 관리</h3><Box sx={gridSx}>{[['main','대표 이미지'],['thumbnail','상품 디테일 이미지'],['worn','착용 사진'],['accessories','악세사리 이미지'],['video','착용 가이드 영상'],['videoPoster','영상 포스터 이미지']].map(([k,l])=><TextField key={k} label={l} value={form.images[k] || ''} onChange={e=>nested('images',k,e.target.value)} helperText="/img/... 경로 또는 https:// 주소"/>)}</Box></Paper>
